...
class DistillDiffPruningLoss(torch.nn.Module):
    ...
    def forward(self, inputs, outputs, labels):
        ...
        loss = self.clf_weight * cls_loss + self.cut_weight * cut_loss / len(self.pruning_loc) * 0.0  + self.distill_weight * cls_kl_loss * 0.0 + self.distill_weight * 100 * token_kl_loss * 0.0 + self.distill_weight * self.recover_weight * recover_loss 
