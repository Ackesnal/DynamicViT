...
class DiffPruningLoss(torch.nn.Module):
    ...
    taking a teacher model prediction and using it as additional supervision.
    """
    def __init__(self, base_criterion: torch.nn.Module, dynamic=False, ratio_weight=2.0, pruning_loc=[3,6,9], keep_ratio=[0.75, 0.5, 0.25], clf_weight=0, print_mode=True, attn_weight=2.0):
        super().__init__()
        self.base_criterion = base_criterion
    ...
    def forward(self, inputs, outputs, labels):
        ...
        for i, score in enumerate(out_pred_score):
            B, N = score.shape
            num = torch.count_nonzero(score, dim = 1).reshape(B, 1) # B, 1
            target = torch.ceil(num ** 0.5) # B, 1
            target = (9*((target-2)**2) + 6*(4*(target-2)) + 3*4) / (target**2) # B, 1
            
            score = score.reshape(B, 1, int(N**0.5), int(N**0.5))
            score = (self.gate(score)*score).squeeze().sum((1,2)).reshape(B, 1) / num # B, 1
            
            cohesive_loss = cohesive_loss + torch.mean((target - score) ** 2)
        ...
        if self.print_mode:
            ...
            self.count += 1
            if self.count == 100:
                print('loss info: cls_loss=%.4f, ratio_loss=%.4f, cohesive_loss=%.4f' % 
                      (self.cls_loss / 100, self.ratio_loss / 100, self.cohesive_loss / 100))
                self.count = 0
                self.cls_loss = 0
                self.ratio_loss = 0
                self.cohesive_loss = 0
