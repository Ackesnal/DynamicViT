...
class Attention(nn.Module):
    ...
    def softmax_with_top_attn(self, attn, num_keep_node):
        B, H, N, N = attn.shape
        weights = torch.ones(N-1).expand(B, -1)
        top_attn = torch.multinomial(weights, num_samples=num_keep_node, replacement=False)
        top_attn.to(device = attn.device)
    ...
    def forward(self, x, num_keep_node = None, test = False):
        if test == True:
            with torch.no_grad():
                B, N, C = x.shape
                weights = torch.ones(N-1, device=x.device).expand(B, -1)
                top_attns = torch.multinomial(weights, num_samples=num_keep_node, replacement=False)
                top_attns.to(x.device)
